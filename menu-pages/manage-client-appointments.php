...
<div class="bs-docs-example tooltip-demo">
    <div style="background:#C3D9FF; margin-bottom:10px; padding-left:10px;">
      <h3><?php _e('Manage Your Appointment(s)','appointzilla'); ?></h3>
    </div>
    ...
    if(isset($_POST['filter'])) {
        ...
        if($FilterData =='today') {
            ...
            $all_appointments = $wpdb->get_results("select * from `$AppointmentTableName` WHERE `email` = '$UserEmail' AND `date` ='$FilterAppointments' ORDER BY `date` DESC limit $offset, $NoOfRow ");
            $cat = $wpdb->get_results("select * from `$AppointmentTableName` WHERE `email` = '$UserEmail' AND `date` ='$FilterAppointments' ORDER BY `date` DESC");
        } else {
            ...
            if($FilterAppointments == 'unpaid' || $FilterAppointments == 'paid') {
                $all_appointments = $wpdb->get_results("select * from `$table_name` WHERE `email` = '$UserEmail' AND `payment_status` ='$FilterAppointments' ORDER BY `date` DESC limit $offset,$NoOfRow");
                $cat=$wpdb->get_results("SELECT * FROM `$table_name` WHERE `email` = '$UserEmail' AND `payment_status` ='$FilterAppointments' ORDER BY `date` DESC");
            } else {
                $all_appointments = $wpdb->get_results("select * from `$table_name` WHERE `email` = '$UserEmail' AND `status` ='$FilterAppointments' ORDER BY `date` DESC limit $offset,$NoOfRow");
                $cat = $wpdb->get_results("SELECT * FROM `$table_name` WHERE `email` = '$UserEmail' AND `status` ='$FilterAppointments' ORDER BY `date` DESC");
            }
        }
        ...
        if($FilterData =='All') {
            ...
            $table_name = $wpdb->prefix . "ap_appointments";
            $all_appointments = $wpdb->get_results("select *  from `$table_name` WHERE `email` = '$UserEmail' ORDER BY `date` DESC limit $offset,$NoOfRow");
            $cat = $wpdb->get_results("select *  from `$table_name` WHERE `email` = '$UserEmail' ORDER BY `date` DESC");
        }
    } else {
            ...
            if(isset($_GET['filtername'])) {
                ...
                if($FilterData=='today') {
                    ...
                    $table_name = $wpdb->prefix . "ap_appointments";
                    $all_appointments = $wpdb->get_results("select * from `$table_name` WHERE `email` = '$UserEmail' AND `date` ='$FilterAppointments' ORDER BY `date` DESC limit $offset,$NoOfRow");
                    $cat = $wpdb->get_results("select *  from `$table_name` WHERE `email` = '$UserEmail' AND `date` ='$FilterAppointments' ORDER BY `date` DESC");
                } else {
                    $FilterAppointments =$FilterData;
                    $table_name = $wpdb->prefix . "ap_appointments";
    ...
    <form action="" method="post" name="manage-appointments">
        <table width="100%" border="0" class="table table-hover">
            ...
            <tr>
                ...
                <td>
                    <a href="?page=update-client-appointment&viewid=<?php echo $appointment->id; ?>" title="<?php /*_e('View','appointzilla'); */?>" rel="tooltip"><i class="icon-eye-open"></i></a>&nbsp;
                    <a href="?page=update-client-appointment&updateid=<?php echo $appointment->id; ?>" title="<?php /*_e('Update','appointzilla'); */?>" rel="tooltip"><i class="icon-pencil"></i></a>&nbsp;
            ...
            <tr>
                <td colspan="8">
                    <ul id="pagination-flickr" style="border:1px #CCCCCC;">
                        ...
                        for($i=1; $i<=$page; $i++) { ?>
                            <li><a href="?page=manage-appointments&pageno=<?php echo $i?>&filtername=<?php echo $FilterData; ?>" <?php if($pgno == $i ) echo "class='active'"; else echo "class=''"; ?>  ><?php echo $i; ?> </a></li>
                        <?php } ?>
          ...
          <tr class="alert"><td colspan="9"><strong><?php _e('Sorry No Appointment(s).','appointzilla'); ?></strong></td>
            <td>&nbsp;</td>
          </tr>
    ...
    function appchangestatus(appid) {
        if(confirm("<?php _e("Are sure to cancel this appointment?", "appointzilla"); ?>")) {
            var Appstatuslist = "#Appstatuslist"+appid;
            ...
            var PostData = "appid=" + appid + "&appstatus=" + status;
            jQuery.ajax({
                type     : "POST",
                cache    : false,
                url      : url,
