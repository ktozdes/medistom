<?php
    ...
    // Get all appointment of current logged in client
    $AppointmentTableName = $wpdb->prefix . "ap_appointments";
    $FetchAllApps_sql = "select `id`, `name`, `start_time`, `end_time`, `date` FROM `$AppointmentTableName` WHERE `recurring` = 'no' AND `email` = '$UserEmail'";
    $AllAppointments = $wpdb->get_results($FetchAllApps_sql, OBJECT);
    //recurring app sql
    $FetchAllRApps_sql = "select * FROM `$AppointmentTableName` WHERE `recurring` = 'yes' AND `email` = '$UserEmail'";
    ...
    if(count($AllAppointments) || count($AllRecurringAppointments)) {
    ?>
        ...
        jQuery('#calendar').fullCalendar({
            ...
            events: [
                ...
                if($AllAppointments) {
                    ...
                    {
                        title: "<?php _e('Appointment By', 'appointzilla'); echo ": ".ucwords($title); ?>",
...
<?php } else {
    ?><br><div class="alert alert-block" style="margin-right: 16px;"><strong><?php _e("Sorry", "appointzilla"); ?></strong> <?php _e("There is no appointment in record schedule by you.", "appointzilla"); ?></div><?php
} ?>